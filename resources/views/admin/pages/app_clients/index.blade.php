...
@section('header-title')
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h2 class="text-capitalize d-inline">all clients</h2>
            </div>
        </div>
    </div><!-- /.container-fluid -->
@endsection
                ...
                @can('clients-list')
                    <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" aria-describedby="example1_info">
                        ...
                        <tr>
                            ...
                            <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">Date Of Birth</th>
                            ...
                            <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">Last donation</th>
                            @can('client-delete')
                        ...
                        <tbody>
                            @foreach ($clients as $key => $client)
                                <tr class="{{$key % 2 == 0 ? 'even' : 'odd'}}}">
                                    ...
                                    <td class="dtr-control sorting_1" tabindex="0">{{$client->date_of_birth}}</td>
                                    <td class="dtr-control sorting_1" tabindex="0">{{$client->email}}</td>
                                    <td class="dtr-control sorting_1" tabindex="0">{{$client->city->name}}</td>
                                    <td class="dtr-control sorting_1" tabindex="0">{{$client->bloodType->name}}</td>
                                    <td class="dtr-control sorting_1" tabindex="0">{{$client->last_donation_date}}</td>

                                    @can('client-delete')
                                        <td>
                                            <button type="button" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#delete-{{$client->id}}">
                                                    ...
                                                    <div class="modal fade" id="delete-{{$client->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                        ...
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <p>Are your sure you want to delete client:{{$client->name}}?</p>
                                    ...
                                    @can('client-toggle-status')
                                        <td>
                                            <a data-toggle="modal" data-target="#change-state-{{$client->id}}" class="btn btn-sm btn-{{$client->is_active == 1 ? 'success' : 'secondary'}} mx-1" title="{{$client->is_active == 1 ? 'active' : 'in active'}}">
                                                <i class="fas fa-toggle-{{$client->is_active == 1 ? 'on' : 'off'}}"></i>
                                            </a>
                                            <!-- Change state modal start -->
                                            <form action="{{route('toggle_state',$client->id)}}" method="POST">
                                                <div class="modal fade" id="change-state-{{$client->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                    <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                        <h5 class="modal-title" id="exampleModalLabel">Change Client Active Status</h5>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                            <span aria-hidden="true">&times;</span>
                                                        </button>
                                                        </div>
                                                        <div class="modal-body">
                                                            <p>Are your sure you want to change active status for :{{$client->name}}?</p>
                                                                @csrf
                                                                <input type="hidden" value="{{$client->id}}" name="id">
                                                                <div class="modal-footer">
                                                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
